...
| 图文学习 | 推荐页 + 专业课程，is-learned 跳过已学 + courseid 全局去重 |
| 视频学习 | 集团视频 / 单位课程 / 案例学习 分类按钮，触屏滑动懒加载，三级标签遍历 |
| 自动刷分 | 集团课程 → 单位课程 → 案例学习 回退链，跨分类 courseid 去重 |
...
#### XTong 的贡献
- **需求与测试**：提出 5 项 TODO 并实测验证，发现事故案例与案例学习重复、平台名称为"单位课程"、播放按钮实际 DOM 结构
- **方向指导**：要求实际跑脚本而非猜测 DOM，推动实测驱动开发

#### Claude (AI Assistant) 的贡献
- **Bug 修复**：5 项全清（is-learned 跳过、触屏滑动、三级标签、分类导航、播放按钮）
- **架构改进**：实例级 courseid 全局去重、前端分类按钮拆分、回退链优化
- **实测驱动**：多次登录平台 dump 真实 DOM 确定选择器和数据结构
- **文档更新**：README/CHANGELOG/关于弹窗同步 v1.6.0，TODO.md 完结
